...
for_impeachment_count <- impeach %>%
  filter(for_impeachment == "YES") %>%
  nrow()

against_impeachment_count <- impeach %>%
  filter(for_impeachment == "NO") %>%
  nrow()

for_impeachment_count
against_impeachment_count
...
mean_trump_vote_percent <- impeach %>%
  group_by(for_impeachment) %>%
  summarize(mean_trump_percent = mean(trump_percent))

mean_trump_vote_percent
...
yes_impeachment_members <- impeach %>%
  filter(for_impeachment == "YES")

won_by_less_than_5_points <- yes_impeachment_members %>%
  filter(margin_flag_2018 == "5_points_or_less") %>%
  nrow()

won_by_5_or_more_points <- yes_impeachment_members %>%
  filter(margin_flag_2018 == "more_than_5_points") %>%
  nrow()

won_by_less_than_5_points
won_by_5_or_more_points
...
# looking at avg nonwhite percentage vs for and against impeachment

nonwhiteness <- impeach %>%
  group_by(for_impeachment) %>%
  summarize(mean_nonwhite = mean(pct_nonwhite))
...
```{r}
  ...
  impeach2 <- impeach %>%
  mutate(
    year_announced = format(as.Date(date_announced), "%Y"),
    month_announced = format(as.Date(date_announced), "%m")
  )
